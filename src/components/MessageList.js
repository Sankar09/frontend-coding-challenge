...
function MessageList() {
  ...
  const debounceClearClick = useCallback(
    debounce((index) => {
      if (index >= 0) {
  ...
  const errors = useMemo(
    () => messages.filter((item) => item.priority == 1).reverse(),
    [messages]
  );
  const warnings = useMemo(
    () => messages.filter((item) => item.priority == 2).reverse(),
    [messages]
  );
  const infos = useMemo(
    () => messages.filter((item) => item.priority == 3).reverse(),
    [messages]
  );
            ...
            <Box component="div" m={1}>
              {errors.map((item, index) => (
                <Message
                  ...
                  key={index}
                  clear={() => {
                    debounceClearClick(messages.indexOf(item));
                  }}
                />
              ))}
            </Box>
